...
class Lambda_Means(Predictor):
    ...
    def get_feature_mean(self, resp, feat_id):
        ...
        for instance in range(0, self.ninstances):
            this_feature[idx] = self.get_feature(instance[idx], feat_id)
        return np.mean(this_feature)

    def initialize(self):
        # will be the mean of all of these
        self.resp = np.ones((self.ninstances, self.K))         
        self.means = np.zeros((self.nfeatures, 1))
        for feature in range(0, nfeatures):
            self.means[feature, 1] = self.get_feature_mean(self, resp, feature)
        pass
    ...
    def compute_distance(self, x, means):
        ...
        for k in range(0, K):
            distances[k] = distance.euclidian(x, means[:,k])
        return distances
